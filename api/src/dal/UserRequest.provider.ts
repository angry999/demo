import { UserRequestDal_generated } from './dal.generated';

/**
 * Data access classes for model in typescript (nest.js) for UserRequest
 * NOTE: EDIT AT WILL, this is intially generated and then left alone. Your changes will not be affected by subsequent code generation runs
 * a request by a user for a change to their account. this is not for customer service type requests but rathe structural ones like becoming a notable investor
 */

/**
 * UserRequest
 * a request by a user for a change to their account. this is not for customer service type requests but rathe structural ones like becoming a notable investor
 */
export class UserRequestDal extends UserRequestDal_generated {
	/**
	 * place your custom code here
	 */
}


