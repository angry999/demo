...
describe('ProjectInterestDal-generated', () => {
    ...
    beforeEach(async () => {
        ...
        dal = await moduleFixture.resolve<ProjectInterestDal>(ProjectInterestDal);
    ...
    it('Find one and interest_by to users_social_profile', async () => {
        let workingResults = await dal.findAllByFilter(null, null, null, null, 1, 10);
        let results = await dal.findAllByFilter(`id = ${workingResults[0].id}`, null, ['interest_by'], null, 1, 10);
        expect(results.length).toBe(1);
        let oneResult = results[0];
        expect(oneResult.id).toBe(workingResults[0].id);
    });
    ...
    it('Find one and interest_in to users_social_profile', async () => {
        let workingResults = await dal.findAllByFilter(null, null, null, null, 1, 10);
        let results = await dal.findAllByFilter(`id = ${workingResults[0].id}`, null, ['interest_in'], null, 1, 10);
        expect(results.length).toBe(1);
        let oneResult = results[0];
        expect(oneResult.id).toBe(workingResults[0].id);
    });
    ...
    it('Create one then update', async () => {
        let newObject = new ProjectInterest();

        // assign value to each property
        // newObject.id = ?; // number
        // newObject.interest_by_id = ?; // number
        // newObject.interest_in_id = ?; // number
        newObject.amount_to_invest = -1; // number 
        newObject.time_until_investment = 'abcdefghijklmnopqrstuvwxyz01234567890abcdefghijklmnopqrstuvwxyz01234567890abcdefghijklmnopqrstuvwxyz01234567890abcdefghijklmnopqrstuvwxyz01234567890abcdefghijklmnopqrstuvwxyz01234567890abcdefghijklmno'; // string 
        ...
        let fkinterest_by = await dalUserSocialProfile.findAllByFilter('', undefined, undefined, undefined, 1, 1); // UserSocialProfile 
        if (fkinterest_by.length > 0)
            newObject.interest_by = fkinterest_by[0] as any; // UserSocialProfile 
        newObject.interest_by_id = (fkinterest_by[0] as any).id;
        let fkinterest_in = await dalUserSocialProfile.findAllByFilter('', undefined, undefined, undefined, 1, 1); // UserSocialProfile 
        if (fkinterest_in.length > 0)
            newObject.interest_in = fkinterest_in[0] as any; // UserSocialProfile 
        newObject.interest_in_id = (fkinterest_in[0] as any).id;
        ...
        expect(savedObject.amount_to_invest).toBe(newObject.amount_to_invest);
        expect(savedObject.time_until_investment).toBe(newObject.time_until_investment);
        ...
        newObject.amount_to_invest = 1; // number
        newObject.time_until_investment = '01234567890abcdefghijklmnopqrstuvwxyz01234567890abcdefghijklmnopqrstuvwxyz01234567890abcdefghijklmnopqrstuvwxyz01234567890abcdefghijklmnopqrstuvwxyz01234567890abcdefghijklmnopqrstuvwxyz01234567890abcd'; // string
        ...
        // compare the two objects
        expect(reloadedObject.amount_to_invest).toBe(newObject.amount_to_invest);
        expect(reloadedObject.time_until_investment).toBe(newObject.time_until_investment);
