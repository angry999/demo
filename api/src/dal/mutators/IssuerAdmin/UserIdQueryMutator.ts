...
/**
 * a query mutator that applied to tables controlled by user id's
 */
export class UserIdQueryMutator extends AffectAllQueryMutator {
    ...
    adjustFor(query: Select, schema: string, table: string, table_alias: string, user: UserIdentification): any {
        let text = `${table_alias}.user_id in (${InvestorQueryMutator.visibleInvestorsIds(schema, table, table_alias, user)})`;
