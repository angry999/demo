import { ProjectPdfDal_generated } from './dal.generated';

/**
 * Data access classes for model in typescript (nest.js) for ProjectPdf
 * NOTE: EDIT AT WILL, this is intially generated and then left alone. Your changes will not be affected by subsequent code generation runs
 * one pdf for one project or project entity
 */

/**
 * ProjectPdf
 * one pdf for one project or project entity
 */
export class ProjectPdfDal extends ProjectPdfDal_generated {
	/**
	 * place your custom code here
	 */
}


