...
/**
 * Informational model in typescript (nest.js) for BackgroundProcess
...
export class BackgroundProcess {
	 ...
	 */	
	@ApiProperty({ description: 'if set, the point in time the process is known to have started'}) 
	started_at: Date;

	/**
	 * if set, the point in time that the process is known to have finished
	 */	
	@ApiProperty({ description: 'if set, the point in time that the process is known to have finished'}) 
	finished_at: Date;

	/**
	 * if true, the process executed successfully
	 */	
	@ApiProperty({ description: 'if true, the process executed successfully'}) 
	succeeded: boolean;
	 ...
	 */	
	@ApiProperty({ type: () => AllUser, description: 'This was created from the details of the foreign key FK_BACKGROUND_PROCESSS_LASTUSER_LAST_CHANGED_BY_ID_CHANGES and described as: '}) 
