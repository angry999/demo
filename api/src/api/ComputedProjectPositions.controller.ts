...
@ApiBearerAuth()
@Controller('ComputedProjectPositions')
@Injectable({ scope: Scope.REQUEST })
export class ComputedProjectPositionsApi extends AbstractController<ComputedProjectPositions>
{
	constructor(@Inject(REQUEST) private readonly request: Request, private readonly service: ComputedProjectPositionsService) {
		super();
	}

	@Get()	
	@ApiOperation({ summary: 'Find all ComputedProjectPositions of AdminLog that match the given filter. If none are found, an empty result is returned' })
	...
	@ApiOkResponse({
		description: 'All instances of ComputedProjectPositions that match the filter',
		isArray: true,
		type: () => ComputedProjectPositions
	})
	...
	): Promise<ComputedProjectPositions[]> {
	...
	@ApiQuery({ name: 'expand', required: false, description: 'a comma separated list of property path\'s that specfiy which additional objects will be filled in the object (deep load). If nothing is specified only the object itself will be loaded, not of its relations will be included' })
	@ApiOkResponse({
		description: 'Get a single instance of ComputedProjectPositions with the specified id',
		type: () => ComputedProjectPositions
	})
	@ApiForbiddenResponse({ description: 'Forbidden.' })
	async findOneById(@Param('id') id: number | string, @Query('expand') expandBy?: string[]): Promise<ComputedProjectPositions> {
		if (this.request['user'] != null)
			this.service.setUserForRequest(this.request['user']);
	...
	@Post()
	@ApiOperation({ summary: 'Create a new instance of ComputedProjectPositions base on the value passed in. If the id is filled in, it will fail to create the new instance, otherwise it returns the id of the newly created object' })
	@ApiBody({ type: ComputedProjectPositions })
	@ApiCreatedResponse({
		description: 'A instance of ComputedProjectPositions has been created, returns that newly created instance',
		type: () => ComputedProjectPositions
	})
	@ApiForbiddenResponse({ description: 'Forbidden.' })
	async create(@Body() newValue: ComputedProjectPositions): Promise<ComputedProjectPositions> {
		if (this.request['user'] != null)
			this.service.setUserForRequest(this.request['user']);
	...
	@Put()
	@ApiOperation({ summary: 'Update the values for the instance of ComputedProjectPositions that is provided' })
	@ApiBody({ type: ComputedProjectPositions })
	@ApiOkResponse({
		description: 'The instance of ComputedProjectPositions has been updated',
		type: () => ComputedProjectPositions
	})
	@ApiForbiddenResponse({ description: 'Forbidden.' })
	async update(@Body() newValue: ComputedProjectPositions) {
		if (this.request['user'] != null)
			this.service.setUserForRequest(this.request['user']);
	...
	@Delete(':id')
	@ApiOperation({ summary: 'Remove the single instance of ComputedProjectPositions with the specified id' })
	@ApiParam({ name: 'id', type: Number, description: 'The id of the instance to remove'})
	@ApiOkResponse({
		description: 'The instance of ComputedProjectPositions has been removed',
		type: () => ComputedProjectPositions
	})
	@ApiForbiddenResponse({ description: 'Forbidden.' })
